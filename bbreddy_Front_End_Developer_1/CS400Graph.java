...
// Email: <your bbreddy@wisc.edu email address>
...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Vertex start) {
      ...
      dataSequence.add(start.data);// add data to dataSequence
      end = start;
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
    ...
    PriorityQueue<Path> path = new PriorityQueue<>();// initialize priority queue
    Path shortestDistance = null;

    Set<Vertex> visitedLoc = new HashSet<>();// storing visited locations in hashset


    if (!(vertices.containsKey(start) && vertices.containsKey(end))) {
      throw new NoSuchElementException();// throw no such element exception when no path found

    }
    Map<T, Integer> pathDistance = initialise(start, path);// initializing path and distance


    while (!path.isEmpty()) {
      Path present = path.remove();
      Vertex presentVertex = present.end;
      if (presentVertex.data.equals(end)) {
        if (shortestDistance == null) {
          shortestDistance = present;
        }

        if (present.distance < shortestDistance.distance) {
          shortestDistance = present;
        }
      }

      if (!visitedLoc.contains(presentVertex)) {

        visitedLoc.add(presentVertex);// if vertex not visited then add vertex

        for (Edge edge : presentVertex.edgesLeaving) {// Traverses through each edge

          if (pathDistance.get(edge.target.data) > pathDistance.get(presentVertex.data)
              + edge.weight) {
            pathDistance.put(edge.target.data, pathDistance.get(presentVertex.data) + edge.weight);
            Path nextPath = new Path(present);
            nextPath.extend(edge);
            path.add(nextPath);
          }
        }
      }
  ...
  /**
   * Private helper method to initialize distances
   *
   * @param start the first node in path
   * @param path  priority queue where data is stored
   * @return Map of all paths
   */
  private Map<T, Integer> initialise(T start, PriorityQueue<Path> path) {

    Map<T, Integer> dis = new HashMap<>();
    for (T t : vertices.keySet()) {
      dis.put(t, Integer.MAX_VALUE);
    }
    Vertex node = vertices.get(start);
    Path initPath = new Path(node);
    path.add(initPath);
    dis.put(node.data, 0);
    return dis;
  }
