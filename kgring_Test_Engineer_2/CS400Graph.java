	...
	*/
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			this.end = source.end;
			this.dataSequence = new LinkedList<>();
			for (T data : source.dataSequence) { // for each loop to make a copy of the list
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		if (!(vertices.containsKey(start))) // if the start vertex doesn't exist
			throw new NoSuchElementException(start + " VERTEX NOT FOUND!");
		if (!(vertices.containsKey(end))) // if the end vertex doesn't exist
			throw new NoSuchElementException(end + " VERTEX NOT FOUND");
		PriorityQueue<Path> discoverQueue = new PriorityQueue<>(); // A priority queue that houses the discovered paths
		Path path = new Path(vertices.get(start)), temp = null; //creates a new path starting at the start node
		Hashtable<T, Path> shortestPath = new Hashtable<>(); // HashTable to store the shortest path between given start and end vertices
		shortestPath.put(start, path);
		discoverQueue.add(path);
		while (!discoverQueue.isEmpty()) {
			path = discoverQueue.remove();
			Iterator<Edge> iter = path.end.edgesLeaving.iterator();
			while (iter.hasNext()) {
				Edge edge = iter.next();
				temp = new Path(path);
				temp.extend(edge);
				if (shortestPath.containsKey(edge.target.data)) {
					if (shortestPath.get(edge.target.data).compareTo(temp) > 0) {
						shortestPath.put(edge.target.data, temp);
						discoverQueue.add(temp);
					}
				} else {
					shortestPath.put(edge.target.data, temp);
					discoverQueue.add(temp);
		...
		if (shortestPath.get(end) == null) // If a path from the start to the end cannot be found
			throw new NoSuchElementException("Path from " + start + " to " + end + " cannot be found.");
