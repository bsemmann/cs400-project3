...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
          ...
          this.dataSequence = new LinkedList<>(source.dataSequence); // make copy of linked list
          this.end = source.end;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
      // check to make sure the requested vertices for a path are actually in the graph
      if (!this.containsVertex(start) || !this.containsVertex(end)) {
        throw new NoSuchElementException("At least one specified node is not in the graph.");
      }
      
      // a vertex is added to this hashtable once the shortest path to it has been identified
      Hashtable<T, Vertex> visitedVertices = new Hashtable<>();
      
      
      PriorityQueue<Path> pathQueue = new PriorityQueue<Path>(); // holds paths to frontier nodes
      pathQueue.add(new Path(vertices.get(start))); // initialize queue by adding the start node
      
      while (!pathQueue.isEmpty()) {
        Path pathToRemove = pathQueue.poll(); // grab the shortest frontier path yet to be taken
        
        if (pathToRemove.end.data.equals(end)) { // check if it's the path that we are looking for
          return pathToRemove;
        }
        
        // continue if a shorter path to this path's end vertex was already identified earlier
        if (visitedVertices.containsKey(pathToRemove.end.data)) {
          continue;
        }
        
        // insert the vertex into the visited vertices hashtable
        visitedVertices.put(pathToRemove.end.data, pathToRemove.end);
        
        // populate the path queue with new frontier paths leading away from this path's end node
        for (Edge e : pathToRemove.end.edgesLeaving) {
          // make sure not to explore paths to vertices we already found a shorter path to
          if (!visitedVertices.containsKey(e.target.data)) {
            Path pathToAdd = new Path(pathToRemove);
            pathToAdd.extend(e);
            pathQueue.add(pathToAdd);
          }
        }
      }
      
      // no path between the two nodes in this direction was identified, so throw an exception
      throw new NoSuchElementException("No path between the two specified nodes was located.");
