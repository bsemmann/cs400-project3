...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		if (!(vertices.containsKey(start))) // if start vertex is not present in the graph
			throw new NoSuchElementException(start + " vertex does not exist");
		if (!(vertices.containsKey(end))) // if end vertex is not present in the graph
			throw new NoSuchElementException(end + " vertex does not exist");

		PriorityQueue<Path> pq = new PriorityQueue<>(); // Priority Queue to store the discovered paths
		Path path = new Path(vertices.get(start)), temp = null;
		Hashtable<T, Path> shortestPath = new Hashtable<>(); // HashTable to store the shortest path between given start
															 // and end vertices
		...
		while (!pq.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				temp = new Path(path);
				temp.extend(e);
				if (shortestPath.containsKey(e.target.data)) {
					if (shortestPath.get(e.target.data).compareTo(temp) > 0) {
						shortestPath.put(e.target.data, temp);
						pq.add(temp);
					}
				} else {
					shortestPath.put(e.target.data, temp);
		...
		if (shortestPath.get(end) == null) // If path from start to end does not exist
