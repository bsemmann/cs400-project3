...
/**
 * This class contains the back end operations used to extract information from
 * a directed graph about the geographical relationships between various cities
 * in Wisconsin.
 * 
 * @author Adam Tupitza
 *
...
public class CitiesBackEnd {
	...
	/**
	 * BackEnd object constructor that initializes the CS400Graph of Strings using
	 * the data loaded by a Cities object.
	 */
	...
	/**
	 * This method determines the best route from a city to another city, if one is
	 * available, and returns an ordered list of all cities to pass through along
	 * that route. The first item in the resulting list is the origin city and the
	 * last item is the destination city. If originCity and destinationCity are
	 * identical, the method returns a 1-item list containing that city's name.
	 * 
	 * @param originCity      city to start the route from
	 * @param destinationCity city to find a route to
	 * @return ordered List of Strings containing the best route from originCity to
	 *         destinationCity
	 * @throws NoSuchElementException if a route between the two cities does not
	 *                                exist, or at least one of the cities is not a
	 *                                vertex in the graph
	 */
	...
	/**
	 * This method determines the best route from a city to another city, if one is
	 * available, and returns the integer value representing the total length in
	 * miles of that route. If originCity and destinationCity are identical, the
	 * method returns 0.
	 * 
	 * @param originCity      city to start the route from
	 * @param destinationCity city to find the distance of the shortest route to
	 * @return an integer representation of the distance in miles of the shortest
	 *         route between originCity and destinationCity
	 * @throws NoSuchElementException if a route between the two cities does not
	 *                                exist, or at least one of the cities is not a
	 *                                vertex in the graph
	 */
	...
	/**
	 * This static method calculates the expected travel time (in minutes) of a
	 * trip.
	 * 
	 * @param tripDistance the length in miles of a proposed trip
	 * @param milesPerHour the expected average speed (in mph) corresponding to a
	 *                     mode of transportation
	 * @return an integer value of the expected travel time of the trip, in minutes
	 * @throws IllegalArgumentException if the entered distance is negative or the
	...
	/**
	 * Given an origin city, this method randomly finds another suggested city in
	 * the graph to visit. The method will only return a suggestion of a city that a
	 * path to from the originCity exists.
	 * 
	 ...
	 * @throws IllegalArgumentException if the originCity is not a vertex in the
	 *                                  graph
	 * @throws IllegalStateException    if no path to another city from the
	 *                                  originCity exists
	 */
	public String getRandomDestination(String originCity) {
		...
		// check to make sure there are multiple vertices in the graph and that at least
		// 1 edge exists
		...
		while (!destinationFound) {
			...
			// loop through the vertices a (randInt + 1) number of times, save the
			// currentCity, and break
			...
			// destination cannot be identical to the origin city, so restart the while loop
			// and try again
			if (currentCity.equals(originCity))
				continue;

			// make sure a route can be identified between the origin city and proposed
			// random destination
			try {
